...
silene <- filter(compadredb,Genus=="Silene") %>% 
  filter(Species=='acaulis') %>% 
  as.data.frame()

silene_output <- NULL

for(ii in 1:length(silene$mat)){
  
  tmp <- silene[ii]
  fullmat <- matA(silene$mat)[[1]]
  growthmat <- matU(silene$mat)[[1]]
  ...
  if(good_growthmat){
    ...
    tmp_df <- data.frame(elevation=tmp$Altitude,
                          mean_longevity=mean_long,
                          max_longevity=max_long,
                          gt = tmp.T,
                          NRR = tmp.NRR,
                          ...
                          eco_region = tmp$Ecoregion,
                          matdim = tmp$MatrixDimension)
    silene_output <- rbind(silene_output,tmp_df)
...
plot(silene_output$elevation,silene_output$mean_longevity)
